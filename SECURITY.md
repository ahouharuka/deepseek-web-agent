...
- Test execution uses a fixed argument array with `shell=False` and a reduced environment that omits common credential variables.
...
- Python tests are executable local code. They can still read files, write files, or use the network with the permissions of the current OS user. Only approve `run_tests` for a workspace you trust.
